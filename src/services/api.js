...
// Axios instance pointing to live backend
const API = axios.create({
...
export const importCsv = (formData) =>
  API.post("/employees/import-csv", formData, {
    headers: { "Content-Type": "multipart/form-data" },
  });
export const sendBulkEmail = () => API.post("/employees/send-bulk-email");
export const sendReminders = () => API.post("/employees/send-reminders");
...
export const getUpcomingEvents = (days = 30) =>
  API.get(`/employees/upcoming-events?days=${days}`);
...
// ✅ Update these to include full backend URL
export const getImageUrl = (filename) =>
  `${process.env.REACT_APP_API_URL}/api/images/${filename}`;
export const getDownloadUrl = (employeeId) =>
  `${process.env.REACT_APP_API_URL}/api/images/download/${employeeId}`;
